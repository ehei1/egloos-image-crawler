...
ImageSize = collections.namedtuple('SIZE', ('WIDTH', 'HEIGHT'))
...
class Crawler:
    __image_size = ImageSize(600, 600)
    __error_code = 0
    ...
    def __init__(self, url, save_path, size=None):
        ...
        assert isinstance(size, (type(None), ImageSize))
    ...
    async def __crawl_post(self, url):
        ...
        for post_view_soup in post_view_soups:
            ...
            if category_soup is not None:
                ...
                category_soup = post_view_soup.find('li', {'class': 'post_info_category'})
                if category_soup is not None:
                    category = category_soup.contents[0].string
                else:
                    raise RuntimeError
            ...
            if children:
                folder_path = self.__get_file_path(self.__save_path, blog_title, category, post_title)
